...
describe("MSW handlers", () => {
  it("GET /events returns the paginated envelope and filters by radius", async () => {
    ...
    const res = await fetch(`${BASE_URL}/events?lat=52.4064&lng=16.9252`);
  ...
  it("GET /events/search filters by query text", async () => {
    const res = await fetch(`${BASE_URL}/events/search?q=techno`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("GET /events filters by category", async () => {
    const res = await fetch(`${BASE_URL}/events?category=nightlife`);
    const body = await res.json();
  ...
  it("GET /events keeps events on both ends of a date-only range", async () => {
    ...
    const sameDay = await fetch(
      `${BASE_URL}/events?date_from=2026-11-24&date_to=2026-11-24`,
    );
    ...
    const range = await fetch(
      `${BASE_URL}/events?date_from=2026-11-25&date_to=2026-11-27`,
    );
    const rangeIds = (await range.json()).data.map(
      (e: { id: string }) => e.id,
    );
  ...
  it("GET /events/search applies category and date filters", async () => {
    const res = await fetch(
      `${BASE_URL}/events/search?q=&category=music&date_from=2026-11-24&date_to=2026-11-24`,
    );
  ...
  it("GET /events/:id returns 404 for an unknown id", async () => {
    const res = await fetch(`${BASE_URL}/events/does-not-exist`);
  ...
  it("GET /venues/:id returns a single venue with snake_case fields", async () => {
    const res = await fetch(`${BASE_URL}/venues/v1`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("GET /venues/:id/events returns the paginated envelope for that venue", async () => {
    const res = await fetch(`${BASE_URL}/venues/v1/events`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body.data)).toBe(true);
    for (const event of body.data) {
